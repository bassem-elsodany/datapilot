import React from 'react';
import { Container, Stack } from '@mantine/core';

import { useTranslation } from '../services/I18nService';
import '../assets/css/components/LoginPanel.css';

interface LoginPanelProps {
  children?: React.ReactNode;
}

export const LoginPanel: React.FC<LoginPanelProps> = ({ children }) => {
  const { tSync } = useTranslation();

  return (
    <div className="login-panel">


      <div className="container">
        <Stack h="100%" justify="flex-start" gap={0}>
          <div className="app-header-section">
            <div className="app-logo">
              <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none">
                <polygon
                  points="4.69 7.7 9.38 4.99 9.38 2.27 2.25 6.38 2.64 6.86 4.26 7.74 4.69 7.7"
                  fill="#4285f4"
                />
                <path
                  d="M9.38,17.36,2.25,13.25V10.66l7.13,4.12Zm0,1.77L2.25,15v2.6l7.13,4.12Zm0-8.71L4.69,7.7,2.25,6.38v2.5L9.38,13"
                  fill="#aecbfa"
                  fillRule="evenodd"
                />
                <path
                  d="M9.38,21.73l7-4V15.1l-7,4m0-4.35,7-4h2.08v2.33H16.78l-7.4,4.29Zm0-1.77L16.79,8.7h2.44v1.36l2.51-2.51-2.51-2.5V6.37H16.36l-1.23.72-1.06.61L11.35,9.27l-2,1.15Z"
                  fill="#669df6"
                  fillRule="evenodd"
                />
                <polygon
                  points="9.38 4.99 12.54 6.82 14.9 5.46 9.38 2.27 9.38 4.99"
                  fill="#669df6"
                  fillRule="evenodd"
                />
              </svg>
              <div className="app-brand">
                <h1 className="app-title animated-title">Data Pilot</h1>
                <div className="app-subtitle animated-subtitle">Your AI pilot for Salesforce data navigation</div>
              </div>
            </div>
            
            <div className="app-description">
              <p className="description-text animated-description">
                {tSync('app.description', 'Advanced SOQL query editor with AI-powered intelligence, schema exploration, result analysis, and smart data navigation.')}
              </p>
            </div>
          </div>
          
                           <div className="content-section">
                   {children || null}
                 </div>
        </Stack>
      </div>
    </div>
  );
};
