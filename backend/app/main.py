...
def create_app() -> FastAPI:
    ...
    async def validation_exception_handler(
        ...
        for error in exc.errors():
            ...
            if error["type"] == "string_too_short":
                field_errors[field_name] = (
                    f"validation.errors.min_length_{error['ctx']['min_length']}"
                )
        ...
        return JSONResponse(status_code=422, content={"detail": error_data})
...
if __name__ == "__main__":
    # For direct execution, use the dedicated runner script instead:
    # python run_server.py
    import uvicorn

    logger.warning(
        "Running via main.py directly. Consider using 'python run_server.py' for better control."
    )
    uvicorn.run(
